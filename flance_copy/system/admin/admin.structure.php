...
else
{
	...
	if ($a == 'update' && !empty($_POST))
	{
		...
		$rstructuredescext = cot_import('rstructuredescext', 'P', 'ARR');
		...
		foreach ($rstructurecode as $i => $k)
		{
			...
			$rstructure['structure_desc'] = cot_import($rstructuredesc[$i], 'D', 'TXT');
                        $rstructure['structure_descext'] = cot_import($rstructuredescext[$i], 'D', 'TXT');
	...
	elseif ($a == 'add' && !empty($_POST))
	{
		...
		$rstructure['structure_desc'] = cot_import('rstructuredesc', 'P', 'TXT');
                $rstructure['structure_descext'] = cot_import('rstructuredescext', 'P', 'TXT');
	...
	foreach ($sql->fetchAll() as $row)
	{
		...
		$t->assign(array(
			...
			'ADMIN_STRUCTURE_DESC' => cot_inputbox('text', 'rstructuredesc['.$structure_id.']', $row['structure_desc'], 'size="64" maxlength="255"'),
                        'ADMIN_STRUCTURE_DESCEXT' => cot_inputbox('text', 'rstructuredescext['.$structure_id.']', $row['structure_descext'], 'size="32"'),
	...
	if (!$id && empty($al))
	{
		...
		$t->assign(array(
			...
			'ADMIN_STRUCTURE_DESC' => cot_inputbox('text', 'rstructuredesc', null, 'size="64" maxlength="255"'),
                    'ADMIN_STRUCTURE_DESCEXT' => cot_inputbox('text', 'rstructuredescext', null, 'size="64"'),
